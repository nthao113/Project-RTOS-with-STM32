...
int main(void)
{
  ...
  osThreadDef(Task1, Task1_function, osPriorityHigh, 0, 128);
  ...
  osThreadDef(Task3, Task3_function, osPriorityNormal, 0, 128);
...
void Task1_function(void const * argument)
{
  ...
  for(;;)
  {
    osSemaphoreWait(CountSemHandle, osWaitForever);// dowm: tru bien dem di 1 neu be hon 0 >> blocked
		{
    ...
    if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_5)==1)
		{
			...
			while(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_5)==1) {};// chong doi
		}
		if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_6)==1)
		{
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4,0);
			while(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_6)==1) {};// chong doi 
		}
		osDelay(100);
  }
...
void Task3_function(void const * argument)
{
  ...
  for(;;)
  {
		...
		if(check==0)
		{		
			osSemaphoreRelease(CountSemHandle); //Up: tang bien dem len 1 .coutsem = 1>1
		}
		osDelay(100);
  }
